...
from data_utils.get_dataset_new import ODIR_CLASS,RFMiD_CLASS,Kaggle_CLASS
import numpy as np
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=0,
                 ...
                 rm_self_attn_dec=True, rm_first_self_attn=True, dataname="odir"):
        super().__init__()
        ...
        self.decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                          return_intermediate=return_intermediate_dec,dataname=dataname)
        ...
        if dataname == 'odir':
            self.pro_m_t = nn.Linear(49, 12)
        elif dataname == 'rfmid':
            self.pro_m_t = nn.Linear(49, 20)
        elif dataname == 'kaggle':
            self.pro_m_t = nn.Linear(49, 50)
        self.att = nn.Sigmoid()
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False,dataname="odir"):
        super().__init__()
        ...
        if self.dataname == "odir":
            self.hier_class = [12, 8]
        elif self.dataname == "rfmid":
            self.hier_class = [20, 16, 6, 3]
        elif self.dataname == "kaggle":
            self.hier_class = [50, 17]
    ...
    def forward(self, tgt, memory,
        ...
        for layer in self.layers:
            ...
            # in odir, the label tree is 2-layer, therefore we only need to calculate the first layer
            if (self.dataname == 'odir' or self.dataname == 'rfmid' or self.dataname == 'kaggle' ) and curr_layer == 1:
                if self.dataname == 'odir':
                    coarse_CLASS = ODIR_CLASS
                elif self.dataname == 'rfmid':
                    coarse_CLASS = RFMiD_CLASS
                elif self.dataname == 'kaggle':
                    coarse_CLASS = Kaggle_CLASS
                for i in range(len(coarse_CLASS)):
                    length_value = len(coarse_CLASS[str(i)]) # get the length of the sub-class per class
                    a = coarse_CLASS[str(i)] # get the current index of the sub-class
                    inter = output_inter[a[0]] # as a base, calculate the average of the sub-class embedding
                    # calculate the average of the sub-class embedding
                    if length_value != 1:
                ...
                query_pos = output_inter_trans # (8,64,2048)
